...
当前项目状态唯一以 [`docs/PROJECT_CONTROL.md`](../../PROJECT_CONTROL.md) 为准。最近任务读取 [`docs/TASK_HANDOFF.md`](../../TASK_HANDOFF.md)，架构和业务决策读取 [`docs/DECISION_LOG.md`](../../DECISION_LOG.md)，一次性目标审计读取 [`docs/GOAL_ALIGNMENT_AUDIT.md`](../../GOAL_ALIGNMENT_AUDIT.md)，真实变更读取 [`CHANGELOG.md`](../../../CHANGELOG.md)。`COMMANDER_STATE.md`、`COMMANDER_DECISIONS.md` 和历史进度百分比仅作历史参考。
...
所有 Sol、Terra、Luna 任务在派发前必须遵守 [MODEL_TASK_ALLOCATION_RULES.md](../../governance/MODEL_TASK_ALLOCATION_RULES.md)。模型按任务难度、风险和重复程度分配，优先使用能够可靠完成任务的最低等级模型，禁止重复开发和无理由浪费高等级模型算力。
