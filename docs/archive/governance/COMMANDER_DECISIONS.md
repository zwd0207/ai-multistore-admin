...
> 历史记录说明（2026-07-19）：本文件保留历史决策，不再作为当前决策入口。当前状态以 [`docs/PROJECT_CONTROL.md`](../../PROJECT_CONTROL.md) 为准，新增决策只追加到 [`docs/DECISION_LOG.md`](../../DECISION_LOG.md)。
...
## D044 - T22 production cutover automated acceptance passed

On 2026-07-17, release `820bfab` completed the approved production cutover. GitHub Actions run `29572971660`, local full verification, Sol security review, the server production build, an actual-data disposable migration, and an isolated production runtime all passed before service interruption. The final stopped-service SQLite backup was verified twice and copied to a second protected local location before the PostgreSQL target was changed.

PostgreSQL was upgraded exactly once to `d4b7a91c2e6f`. The migration copied 29 source tables with matching canonical digests, preserved 2 stores, 18 orders, 10 products, and 2 credentials, validated all constraints, decrypted every migrated credential, and did not migrate sessions. A bootstrap platform-administrator invitation was created without sending email or exposing its token in Git or command output.

An activation precheck initially referenced the wrong invitation column and triggered the prepared rollback before any directory swap. The old SQLite service restarted successfully. It was stopped again, and a fresh source snapshot matched all 29 migrated table digests before activation resumed. The final backend and frontend swap then passed local and public health, HTTPS redirect, new-asset, unauthenticated-access, PostgreSQL revision, and zero-error-log checks. The complete previous backend, frontend, and SQLite source remain preserved for rollback.

The first production PostgreSQL backup was dumped, age-encrypted, uploaded with private object ACL and OSS AES256, and verified by remote size and SHA-256 readback. A full restore drill downloaded that object, verified its manifest, decrypted it only in a protected temporary directory, restored all 48 tables into a disposable PostgreSQL database, matched every production table count, validated the schema and credentials, and removed all plaintext and temporary database material. The hourly timer is active.

T22 automated cutover checks pass, but final operator acceptance remains open: the owner must accept the protected invitation and enroll TOTP MFA, then Commander must verify authenticated tenant/store behavior and desktop/mobile UI. The OSS role must also gain `DeleteObject` on the backup prefix so the two private preflight objects can be removed and retention cleanup can be accepted. No real Naver read, platform write, customer reply, shipment dispatch, email send, Ziniao server call, or AI operation was enabled by this cutover.

## D045 - T22 final authenticated acceptance passed

The owner accepted the one-time platform-administrator invitation, set the password, enrolled TOTP MFA, and retained the recovery codes. Production database evidence shows one active platform administrator, one accepted invitation, no pending invitation, ten unused recovery codes, and an active MFA-verified session with an explicit tenant selection. The bootstrap shell had stored the Chinese display name as six ASCII question marks; Commander corrected that single field with Unicode code points and verified the intended name in PostgreSQL and the authenticated UI without changing identity, MFA, session, tenant, or permission data.

Authenticated browser acceptance passed at 1440x900 and 390x844. The workbench rendered the migrated two-store data, switching stores changed the visible order set, and orders, shipping, customer service, store connection, and tenant invitation routes loaded under the same MFA session. The refreshed console contained zero errors or warnings, desktop and mobile roots had no horizontal overflow, and platform write controls remained closed. A mobile regression was found where the More overlay survived primary-route navigation; commit `f3036e3` closes it on every pathname change, adds a contract guard, passes the complete local verification and bundle budget, and passed GitHub Actions run `29582534495` before an atomic static-frontend deployment.

The OSS role now has the required prefix-scoped `DeleteObject` permission. The two private preflight objects were deleted, a fresh upload/read/delete probe passed, and the hourly age-encrypted PostgreSQL backup passed again. The protected cutover result records authenticated acceptance and OSS deletion as passed, and its regenerated SHA-256 manifest verifies all seven evidence files. After final QA, both protected copies of the consumed invitation token were deleted. T22 is final-accepted; this does not enable real Naver reads, lifecycle schedulers, email delivery, Ziniao server integration, customer replies, shipment dispatch, inventory/product mutation, platform writes, or AI operations.

## D046 - T24 PXG inquiry capability validated with one GET

The owner selected the exact production PXG Naver store for the first T24 validation. Commander resolved it to the active store and its sole active encrypted credential, confirmed Client ID, Secret, API base and channel configuration without exposing their values, found no active inquiry lease or recent rate-limit cooldown, and verified that real API testing, automatic synchronization, persistent inquiry reads, every platform-write flag, customer replies, shipment dispatch, inventory/product mutation, and AI operations were all disabled.

Commit `adacd19` adds a reusable one-shot probe instead of temporarily opening the HTTP refresh endpoint. The probe requires process-local owner approval, exact store ID and store-name hash, one active credential, closed runtime gates, no lease, and no current 429 cooldown. It performs one SELF token exchange and exactly one `GET /v1/pay-user/inquiries` request for page 1 with size 10. It never retries, follows pagination, saves raw payloads, writes inquiry content, or calls a platform-write path. Focused T24 and production-session tests, the complete local `verify_all.py`, and GitHub Actions run `29584540587` passed before production execution.

The production probe returned HTTP 200, confirmed the `order_seller` grant, observed 10 first-page items and 35 total items across four pages, and deliberately issued zero follow-up GETs. It persisted only a safe `ApiCapabilityTestResult`; order, product, ordinary inquiry, protected readonly-inquiry, and checkpoint counts remained unchanged. The temporary executable was hash-verified and removed after use. This evidence changes the capability contract to `safe_to_real_test=true`, `grant_confirmed=true`, and token type `SELF`, but it does not authorize the remaining three pages, the initial 30-day import, persistent refresh, automatic scheduling, or any platform write. Those remain separate owner gates.

Capability acceptance commit `d175991` passed the complete local verification and GitHub Actions run `29585205365`, then replaced only the production capability-metadata module under a retained rollback copy. The deployed SHA-256 matched the release file, compilation and HTTPS health passed, the runtime imported the expected `True / True / SELF / None` contract, all four production services remained active, and the post-restart error journal was empty. Real API testing, persistent inquiry reads, automatic synchronization, customer replies, shipment dispatch, every platform-write flag, and AI operations remained false.

## D047 - T24 one-time PXG inquiry import accepted

The owner separately approved one 30-day import for the exact PXG/Naver store after the one-GET capability probe. The production importer completed with one page read, 35 created protected inquiries, 35 answered classifications, 0 unanswered classifications, and 11 verified legacy duplicates sanitized. The visible list contains exactly 35 records. A repeat invocation was blocked by `t24_import_already_started` before Naver access.

The production quality audit found 35 customer questions, 35 customer names, and 35 answer timestamps. Naver returned historical answer text for 25 records and omitted the optional `answerContent` field for 10 answered records. This is not an unanswered state. Commit `9be5984` adds an explicit `has_answer_content` detail contract and renders those ten records as “已回复” with the Naver answer time and a clear notice that the platform did not provide historical answer text. It never invents answer content.

The owner clarified that operators must be able to read customer information, questions, and replies as ordinary text to handle consultations. The accepted implementation therefore shows the full customer name and conversation after the existing permission-gated detail request. List rows remain masked, while PostgreSQL and encrypted OSS backups retain transparent encryption at rest. This preserves the requested operator experience without storing customer content in plaintext.

Focused tests, frontend build/contracts, synthetic desktop and 390px Playwright QA, full `verify_all.py`, secret-history scanning, and GitHub Actions run `29591010429` passed. The production backend and frontend hashes match the release, rollback copies are retained, public/local health and unauthenticated denial pass, and the service journal has no post-release warnings. A post-import encrypted backup uploaded successfully. Persistent inquiry reads, automatic scheduling, customer replies, shipment dispatch, every platform-write flag, and AI operations remain disabled.

## D048 - Two verified Naver credentials become the production readonly baseline

The owner approved the second existing verified Naver credential as the current system's second production store. This replaces the earlier single-store rollout assumption. Release `39a4794` prepared exactly two stores and eight existing T15 checkpoints, with one-minute resource spacing, a four-minute inter-store offset, exact credential binding, and no network call during preparation.

Production activation completed with the lifecycle scheduler, automatic readonly runtime, and exact two-store inquiry allowlist enabled. Both stores reached `success` for orders, inquiries, and products. Order reads demonstrated resumable cursors and automatic exponential backoff: transient request failures did not reset a cursor, cross stores, or require an operator click. All platform-write and AI gates remained closed. A fresh encrypted PostgreSQL backup was uploaded after the first cycles.

Logistics is not accepted by this decision. Both store checkpoints failed closed with an exact association mismatch and wrote no logistics records. Safe diagnostics found three legacy full-order identifiers in one store and exposed a batch consistency/transaction case that the existing mock contract did not cover. Re-enabling logistics requires a code fix, focused regression tests, full CI, a controlled deployment, and checkpoint-specific recovery; it must not use warehouse writeback or weaken store/product-order matching.

The rollout order is therefore frozen as: repair T17 logistics correctness, prove repeated two-store readonly stability, repeat authenticated desktop/mobile operator QA, then enable invitation/email operations, then trial one manual customer reply, and only afterward trial one manual shipment writeback. Ziniao remains a later Windows-helper concern. This decision authorizes no customer reply, shipment dispatch, inventory/product/order mutation, bulk write, or AI operation.

## D049 - T24 dual-store logistics recovery accepted

Commits `2507c84` and `62e49ba9cfdd19a6f6b026c25d53fc3f2ad98401` repair the production logistics association and source-time paths, prevent partial batch commits, make tracking retention conditional and concurrency-safe, serialize retention against warehouse-batch creation, preserve shared cleanup failures atomically, and harden recover/close/reopen with stopped-service and structured-result gates. No duplicate table, scheduler, logistics API, warehouse flow, or platform-write path was added.

Exact-commit GitHub Actions run `29611088282` passed secret history, frontend, and PostgreSQL 16 backend jobs. Local full verification and two independent security reviews also passed. A hash-verified release was deployed with the previous backend retained at `/srv/codex1-backend.rollback-pre-62e49ba-20260717T203841Z`. Offline production readback proved that both recovery and close marker pairs were complete, both checkpoints were rollback-closed, orders were fresh, no lease existed, and every write/AI gate was false, so the one-time `reopen` transition was used.

The first repaired production cycles completed successfully for both stores and persisted 23 plus 21 encrypted logistics records. A privacy scan found no full tracking number in ordinary order metadata, status events, sync logs, audits, or the service journal. Local/public health and the warning journal passed. Encrypted OSS backups were remotely verified before and after the transition.

This accepts Naver logistics readonly correctness for the two approved production stores. It does not authorize a customer reply, shipment dispatch, order/product/inventory mutation, bulk write, or AI action. The next gate is repeated dual-store stability plus authenticated desktop and 390px operator QA; manual customer reply remains the first future write trial, with shipment writeback evaluated only afterward under a separate exact-record approval.
