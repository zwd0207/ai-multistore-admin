...
- Production cutover completed on `2026-07-17` from the approved `19:00 Asia/Shanghai` window. The production backend is now exact release `62e49ba9cfdd19a6f6b026c25d53fc3f2ad98401`; the production frontend still includes authenticated-acceptance hotfix `f3036e32321756997e5b54f6bd720289b97d6838`. GitHub Actions run `29611088282`, local `verify_all.py`, PostgreSQL concurrency, server compile/tree comparison, structured reopen, and production health all passed.
...
- HTTPS, HTTP redirect, production session configuration, unauthenticated 401 behavior, frontend assets, and post-activation logs pass. Automatic Naver readonly scheduling is enabled only for the two approved stores; email delivery, Ziniao server integration, platform writes, customer replies, shipment dispatch, inventory/product/order mutation, and AI operations remain disabled.
...
- Keep real platform writes, customer replies, and shipment dispatch disabled. The owner-selected PXG probe, one-time 30-day inquiry import, and exact two-store automatic readonly rollout passed. Do not repeat the import or expand automatic synchronization beyond the two approved stores without a new explicit approval.
- T24 controlled production evidence: commit `adacd19` adds a one-shot probe guarded by exact store/name hash, one active credential, closed write/test/scheduler gates, lease and 429 cooldown checks, one business GET, zero follow-up pages, safe capability evidence, and unchanged business counts. GitHub Actions run `29584540587` passed. Production returned HTTP 200 for page 1 size 10, reported 35 total inquiries across four pages, confirmed `order_seller`, saved no raw response or inquiry content, and left all automatic-read and platform-write settings false. Capability acceptance `d175991` passed GitHub Actions run `29585205365` and is deployed; public health, source hash, runtime metadata, service status, and zero recent error logs passed after restart.

## T24 One-Time Inquiry Import Evidence (2026-07-17)

- The owner approved one exact PXG/Naver import for the latest 30 KST business days. The importer used one page at size 200 and created 35 protected readonly inquiry records. Classification is 35 answered and 0 unanswered; no platform write, reply, automatic schedule, raw-response persistence, or retry was enabled.
- All 35 records have a unique valid inquiry hash, customer question, customer name, and answer timestamp. Naver supplied historical answer text for 25 records. For 10 answered records the official optional `answerContent` field was absent; commit `9be5984` keeps them classified as answered and shows the answer time plus an explicit platform-content-unavailable notice instead of falsely marking them unanswered.
- Eleven matching legacy generic inquiries were superseded and sanitized only after encrypted-copy verification. Visible inquiry count remains 35, legacy plaintext columns and sensitive raw keys are zero, encryption-bundle violations are zero, retention violations are zero, and cleanup health is `healthy`.
- Authorized detail reads decrypt and display the full customer name, customer question, store answer, and timestamps as ordinary text for the operator. List rows keep the masked customer name. PostgreSQL and OSS backups remain encrypted at rest; encryption is transparent to the operator and does not restrict reading or future manual reply work.
- Re-running the one-time importer returned `t24_import_already_started` before platform access. A fresh encrypted PostgreSQL backup uploaded successfully after import. Automatic inquiry reads and every real-write, customer-reply, shipment, product, inventory, order, and AI flag remain false.
- Commit `9be5984` passed focused T14/T24 tests, frontend contracts/build, local desktop and 390px synthetic-data Playwright QA, full `verify_all.py`, and GitHub Actions run `29591010429`. Production deployment retained `/srv/codex1-backend.rollback-pre-9be5984-20260717T231422` and `/srv/ai-multistore-admin/dist.rollback-pre-9be5984-20260717T231422`; source hashes, local/public health, unauthenticated 401, all four services, and the post-restart warning journal passed.

## T24 Dual-Store Automatic Read Evidence (2026-07-18)

- The owner approved both existing verified Naver credentials as the first two production stores. Release `39a4794` is deployed from `release/operator-v1`; GitHub Actions run `29597962129` passed secret-history, frontend, PostgreSQL concurrency, and full backend verification.
- Exactly eight existing T15 checkpoints were prepared for two stores and four resources. Activation used one-minute resource spacing and a four-minute store offset. The preparation made no network call, saved no raw response, and enabled no platform write.
- Both stores reached `success` for recent orders, customer inquiries, and products. Bounded order requests encountered transient failures, retained their cursors, followed exponential backoff, and completed without manual replay. The second store imported its approved recent inquiry window and retained answered/open classification.
- Both logistics checkpoints failed closed with `naver_logistics_external_order_id_mismatch`. No logistics record was committed. Safe diagnostics identified legacy full-order identifiers in one store and a separate batch consistency risk that requires a tested compatibility and transaction fix before logistics can be re-enabled.
- The production API remains healthy. Platform writes, customer replies, shipment dispatch, product/inventory mutation, order mutation, and AI operations remain disabled. The post-activation warning journal is empty.
- The service account could not read the root-only `0600` environment file during the maintenance restart. Ownership was corrected to `root:aiops` with mode `0640`; the API then started before the activation window and the protected file remains unreadable to other users.
- A new age-encrypted PostgreSQL backup uploaded to private OSS after the initial cycles. The hourly backup timer remains enabled and active.
- Fresh unauthenticated desktop and 390px production login QA passed HTTPS rendering and mobile containment (`scrollWidth == 390`). Authenticated data-state QA from D045 remains the latest completed authenticated browser evidence; repeat it after the logistics fix.

## Current Execution Goals

1. **P0 dual-store stability:** observe both stores through repeated order, inquiry, product, and repaired logistics cycles; require cursor completion, zero cross-store effects, safe retry behavior, and no privacy or retention regression.
2. **P1 operator readiness:** repeat authenticated desktop and 390px QA against the real two-store data state, including store switching, inquiry classification/conversation display, automatic-read attention, and logistics freshness messaging.
3. **P1 invitations and email:** enable the already implemented invitation, MFA, password-reset, and alert-mail flow only after the dual-store readonly gate remains stable. Keep public self-registration closed for the first operator rollout.
4. **P2 manual writes:** accept customer reply first, then shipment writeback as a separate gate. Each requires its own real single-record approval, idempotency/reconciliation proof, backup health, and all readonly resources fresh. Never enable both for an initial trial.
5. **P3 Windows Ziniao helper:** keep T19/T20 local-only until the core Naver operator workflow is accepted. Do not make Ziniao desktop opening a production-server dependency.

## T24 Dual-Store Logistics Recovery Evidence (2026-07-18)

- Commits `2507c84` and `62e49ba9cfdd19a6f6b026c25d53fc3f2ad98401` close logistics source-time, retry, retention, batch-transaction, rollback, and recovery races. Two independent reviews ended with no P0/P1 finding. Local full `verify_all.py`, all frontend contracts, encoding, production build, session security, and bundle budget passed.
- GitHub `Verify release candidate` run `29611088282` passed secret history, frontend contracts/build, and PostgreSQL 16 backend verification for exact commit `62e49ba`. The release archive SHA-256 is `b4034820b4c5c889a77f73f89b720d602c9f4c11745aacce68e200dc26a13435` and the server extracted tree matched `/srv/releases/operator-v1-62e49ba` exactly.
- Before deployment, both logistics checkpoints were disabled with `t24_logistics_rollback_closed`, recovery/close markers were complete, reopen/reclose markers were absent, leases and next-run times were empty, orders were fresh, and every platform-write and AI flag was false. This proved `reopen`, not a second recovery, was the only valid action.
- The prior backend is retained at `/srv/codex1-backend.rollback-pre-62e49ba-20260717T203841Z`. Alembic remained at head `7e2a9c4f1b36`. The structured reopen result scheduled exactly two checkpoints with 60-second spacing and reported `network_called=false`, `platform_write=false`, and `records_deleted=false` before the API service was started.
- Both stores completed the first real repaired logistics cycle at `success` with no error, retry, or lease. Store 1 saved 23 records and store 2 saved 21. Local and public HTTPS health pass, the post-start warning journal is empty, and all platform-write, customer-reply, shipment-dispatch, product/inventory/order mutation, and AI flags remain false.
- An in-memory privacy scan decrypted all 44 protected tracking numbers without exposing them, found zero plaintext matches in ordinary orders, status events, sync logs, operation audits, or the systemd journal, and found zero mask failures. No platform write occurred.
- Encrypted OSS backups were remotely verified immediately before and after recovery. Pre-recovery backup `postgres-20260717T203237Z-a273077ea6e3` has SHA-256 `3167cb595c7080c5d8f2453ab32593017d23a615314f2e4c090c3d2a7cdabe76`; post-recovery backup `postgres-20260717T204344Z-71d94c419814` has SHA-256 `8998e1f74230c756862dfbd05edeca2d3c33396c79c355735caf7315230fc319`.
