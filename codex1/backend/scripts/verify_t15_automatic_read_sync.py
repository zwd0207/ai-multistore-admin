...
from app.services.automatic_read_sync_service import RESOURCE_CONFIG, automatic_read_status, ensure_onboarded_store_schedules, run_automatic_checkpoint
...
class Reader:
    ...
    def read_products(self, context, *, start_at, end_at, cursor):
        if self.fail: raise NaverReadFailure("network_timeout", retryable=True)
...
def main():
    ...
    with SessionLocal() as db:
        ...
        assert automatic_read_status(db, store_id=stores[0].id)["logistics"]["error_code"] == "not_supported"
