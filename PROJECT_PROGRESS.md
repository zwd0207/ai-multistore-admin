...
### Latest update after `Phase ERP-Multistore-1K` to `Phase Naver-Order-Batch-1F`

- Project overall planning progress: about `82% - 87%`
- Naver basic ERP loop progress: about `89% - 94%`
- ERP real-user landing progress: about `84% - 89%`
- Production version for long-term non-technical use: about `78% - 83%`

Implemented in this update:

- Confirmed the Accounts store-membership readonly panel as a walkthrough target for backend and mock modes.
- Planned the future real user invitation approval boundary without creating users or memberships.
- Planned the future product rollback readonly report UI boundary.
- Added a private Codex1 batch approval audit evidence mock gate.
- Verified the gate requires audit-chain planning, store scope, approval actions, duplicate checks, and field whitelist evidence.
- Planned the Naver order batch approval evidence audit-readiness boundary.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, rollback report planning, and mock-proven audit readiness for future batch approvals.
- Not suitable now: opening formal product/order batch sync, broad product writes, active multi-user login production use, real user invitation, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Multistore-1M: Real user invitation mock gate`
2. `Phase ERP-Multistore-1N: User invitation readonly API plan`
...
### Latest update after `Phase ERP-Multistore-1O` to `Phase Naver-Order-Batch-1H`

- Project overall planning progress: about `84% - 89%`
- Naver basic ERP loop progress: about `91% - 96%`
- ERP real-user landing progress: about `86% - 91%`
- Production version for long-term non-technical use: about `80% - 85%`

Implemented in this update:

- Added a local readonly user-invitation API endpoint in Codex1.
- Added Codex2 data-provider support and mock fallback for user-invitation readonly checks.
- Planned the future product rollback readonly report backend route boundary.
- Added a service-level mock gate for the future batch approval audit-evidence local route.
- Added a Codex2 Orders panel for Naver order batch audit readiness with business-first wording.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, rollback report mock display, mock-proven invitation readiness, and readonly user-invitation readiness checks.
- Not suitable now: opening formal product/order batch sync, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Multistore-1Q: User invitation readonly UI plan`
2. `Phase ERP-Multistore-1R: User invitation readonly UI display`
3. `Phase ERP-Batch-1Q: Batch approval audit evidence local route plan`
4. `Phase Naver-Product-Batch-1P: Product rollback readonly report backend route mock gate`
5. `Phase Naver-Order-Batch-1I: Order batch audit readiness UI walkthrough`

### Latest update after `Phase ERP-Multistore-1Q` to `Phase Naver-Order-Batch-1I`

- Project overall planning progress: about `85% - 90%`
- Naver basic ERP loop progress: about `92% - 96%`
- ERP real-user landing progress: about `87% - 92%`
- Production version for long-term non-technical use: about `81% - 86%`

Implemented in this update:

- Planned and implemented the Accounts-page user invitation readonly UI.
- Added a separate user invitation readiness panel for backend and mock modes.
- Documented the next batch approval audit-evidence local route boundary.
- Added a Codex1 mock gate for the future Naver product rollback readonly report backend route.
- Walked forward the Orders-page audit readiness UI as a readonly business display target.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, rollback report mock display, mock-proven invitation readiness, readonly user-invitation readiness checks, and Accounts/Orders readiness panels.
- Not suitable now: opening formal product/order batch sync, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-1R: Batch approval audit evidence readonly route mock gate`
2. `Phase ERP-Batch-1S: Batch approval audit evidence readonly route implementation`
...
### Latest update after `Phase ERP-Batch-1T` to `Phase ERP-Multistore-1T`

- Project overall planning progress: about `87% - 92%`
- Naver basic ERP loop progress: about `94% - 97%`
- ERP real-user landing progress: about `89% - 94%`
- Production version for long-term non-technical use: about `83% - 88%`

Implemented in this update:

- Planned and implemented the Orders UI integration for the batch approval audit-evidence readonly route.
- Added frontend service support for `POST /api/v1/batch/approval-audit-evidence`.
- Planned and implemented the Products UI integration for the Naver product rollback readonly report route.
- Added frontend service support for `POST /api/v1/batch/naver/products/rollback-readonly-report`.
- Cleaned the Accounts user invitation readonly panel into clearer production-facing Chinese wording.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly user-invitation readiness checks, Accounts/Orders readiness panels, local batch approval audit-evidence route review, local product rollback report route review, and frontend display of those route results.
- Not suitable now: opening formal product/order batch sync, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-1V: Batch approval audit evidence UI runtime walkthrough`
2. `Phase Naver-Product-Batch-1U: Product rollback readonly report UI runtime walkthrough`
...
### Latest update after `Phase ERP-Batch-2A` to `Phase ERP-Multistore-2A`

- Project overall planning progress: about `89% - 94%`
- Naver basic ERP loop progress: about `95% - 98%`
- ERP real-user landing progress: about `91% - 95%`
- Production version for long-term non-technical use: about `85% - 90%`

Implemented in this update:

- Consolidated the formal product/order batch sync operator checklist.
- Added a readonly Orders-page formal batch sync operator checklist panel.
- Planned the Naver product batch approval evidence linkage UI.
- Added a readonly Products-page product batch approval evidence linkage panel.
- Planned the future real user invitation production gate while keeping real invitations closed.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly user-invitation readiness checks, Accounts/Orders readiness panels, local batch approval audit-evidence route review, local product rollback report route review, frontend display of those route results, clearer invitation approval/audit readiness display, and operator-facing formal batch checklist review.
- Not suitable now: opening formal product/order batch sync, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2C: Formal batch checklist runtime walkthrough`
2. `Phase Naver-Product-Batch-2C: Product evidence linkage runtime walkthrough`
...
### Latest update after `Phase ERP-Batch-2F` to `Phase ERP-Multistore-2D`

Progress snapshot:

- Project overall planning progress: about `91% - 95%`
- Naver basic ERP loop progress: about `96% - 98%`
- ERP real-user landing progress: about `92% - 96%`
- Production version for long-term non-technical use: about `87% - 91%`

Implemented in this update:

- Added Codex1 formal batch approval decision mock gate.
- Planned the future readonly approval-decision API without exposing a route.
- Added Codex2 Orders readonly approval-decision panel.
- Added Codex1 real-user invitation approval checklist mock gate.
- Planned the future Accounts invitation approval checklist UI.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly user-invitation readiness checks, Accounts/Orders readiness panels, local batch approval audit-evidence route review, local product rollback report route review, frontend display of those route results, clearer invitation approval/audit readiness display, operator-facing formal batch checklist review, planned formal approval decision records, and mock-proven final approval-decision and invitation-checklist gates.
- Not suitable now: opening formal product/order batch execution, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2I: Formal batch approval decision readonly API mock gate`
2. `Phase ERP-Batch-2J: Formal batch approval decision readonly API local implementation plan`
3. `Phase ERP-Multistore-2E: Invitation approval checklist readonly API plan`
4. `Phase ERP-Multistore-2F: Invitation approval checklist readonly UI mock display`
5. `Phase Naver-Order-Batch-2A: Formal order batch execution approval boundary plan`

### Latest update after `Phase ERP-Batch-2I` to `Phase Naver-Order-Batch-2A`

Progress snapshot:

- Project overall planning progress: about `92% - 96%`
- Naver basic ERP loop progress: about `96% - 98%`
- ERP real-user landing progress: about `93% - 97%`
- Production version for long-term non-technical use: about `88% - 92%`

Implemented in this update:

- Added Codex1 formal batch approval-decision readonly API mock gate.
- Planned the local readonly approval-decision API route without implementing the route.
- Planned the invitation approval checklist readonly API.
- Added Codex2 Accounts invitation approval checklist mock display.
- Planned the Naver order batch execution approval boundary.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly user-invitation readiness checks, Accounts/Orders readiness panels, local batch approval audit-evidence route review, local product rollback report route review, frontend display of those route results, operator-facing formal batch checklist review, planned formal approval decision records, mock-proven final approval-decision gates, and invitation checklist UI mock review.
- Not suitable now: opening formal product/order batch execution, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2K: Formal batch approval decision readonly API local route mock gate`
2. `Phase ERP-Batch-2L: Formal batch approval decision readonly API local implementation`
...
### Latest update after `Phase ERP-Batch-2M` to `Phase Naver-Order-Batch-2C`

Progress snapshot:

- Project overall planning progress: about `94% - 97%`
- Naver basic ERP loop progress: about `97% - 99%`
- ERP real-user landing progress: about `95% - 98%`
- Production version for long-term non-technical use: about `90% - 94%`

Implemented in this update:

- Planned Codex2 integration for the approval-decision readonly API.
- Integrated the Orders formal batch approval decision panel with the route-backed readonly API and mock fallback.
- Added Codex1 invitation approval checklist readonly route mock gate.
- Exposed Codex1 local readonly route `POST /api/v1/permissions/user-invitation/approval-checklist/readonly-check`.
- Planned the future Naver order batch execution approval readonly UI boundary.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly user-invitation readiness checks, Accounts/Orders readiness panels, local approval-decision readonly API review, invitation approval checklist readonly API review, product rollback report review, operator-facing formal batch checklist review, and mock-proven Naver order batch execution approval boundary.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Multistore-2K: Invitation approval checklist readonly API frontend integration plan`
2. `Phase ERP-Multistore-2L: Invitation approval checklist readonly API frontend integration`
3. `Phase Naver-Order-Batch-2D: Order batch execution approval readonly UI implementation`
4. `Phase ERP-Batch-2O: Formal batch approval decision audit linkage plan`
5. `Phase Naver-Product-Batch-2D: Product batch execution approval boundary mock gate`

### Latest update after `Phase ERP-Multistore-2K` to `Phase Naver-Product-Batch-2D`

Progress snapshot:

- Project overall planning progress: about `95% - 98%`
- Naver basic ERP loop progress: about `98% - 99%`
- ERP real-user landing progress: about `96% - 98%`
- Production version for long-term non-technical use: about `91% - 95%`

Implemented in this update:

- Planned Codex2 integration for the invitation approval checklist readonly API.
- Integrated Codex2 Accounts with the route-backed invitation approval checklist readonly API and mock fallback.
- Implemented Codex2 Orders readonly UI for future Naver order batch execution approval readiness.
- Planned audit linkage for future formal batch approval decisions.
- Added Codex1 Naver product batch execution approval mock gate.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly invitation approval checklist API review, Accounts/Orders readiness panels, local approval-decision readonly API review, order/product execution approval mock gates, product rollback report review, and operator-facing formal batch checklist review.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2P: Approval decision audit linkage mock gate`
2. `Phase ERP-Batch-2Q: Approval decision audit linkage readonly API plan`
3. `Phase Naver-Product-Batch-2E: Product batch execution approval UI plan`
4. `Phase Naver-Product-Batch-2F: Product batch execution approval readonly UI implementation`
5. `Phase ERP-Multistore-2M: Invitation approval audit linkage plan`

### Latest update after `Phase ERP-Batch-2P` to `Phase ERP-Multistore-2M`
...
- Naver basic ERP loop progress: about `98% - 99%`
- ERP real-user landing progress: about `96% - 98%`
- Production version for long-term non-technical use: about `92% - 95%`

Implemented in this update:

- Added Codex1 approval-decision audit linkage mock gate.
- Planned the future approval-decision audit-linkage readonly API.
- Planned the Products product-batch execution approval UI.
- Implemented Codex2 Products readonly product-batch execution approval checklist.
- Planned invitation approval audit linkage before any future real user or membership write.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly invitation approval checklist API review, Accounts/Orders/Products readiness panels, local approval-decision readonly API review, order/product execution approval mock gates, product rollback report review, and operator-facing formal batch checklist review.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2R: Approval decision audit linkage readonly API mock gate`
2. `Phase ERP-Batch-2S: Approval decision audit linkage readonly API local implementation plan`
3. `Phase Naver-Product-Batch-2G: Product batch execution approval UI runtime walkthrough`
4. `Phase Naver-Product-Batch-2H: Product batch execution approval readonly API plan`
5. `Phase ERP-Multistore-2N: Invitation approval audit linkage mock gate`

### Latest update after `Phase ERP-Batch-2R` to `Phase ERP-Multistore-2N`

Progress snapshot:

- Project overall planning progress: about `96% - 99%`
- Naver basic ERP loop progress: about `98% - 99%`
- ERP real-user landing progress: about `97% - 98%`
- Production version for long-term non-technical use: about `93% - 96%`

Implemented in this update:

- Added Codex1 approval-decision audit-linkage readonly API mock gate.
- Planned the future local readonly API route for approval-decision audit linkage.
- Documented and walked through the Products product-batch execution approval readonly UI.
- Planned future Naver product batch execution approval readonly API.
- Added Codex1 invitation approval audit-linkage mock gate.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly invitation approval checklist API review, Accounts/Orders/Products readiness panels, approval-decision audit linkage review gates, order/product execution approval mock gates, product rollback report review, and operator-facing formal batch checklist review.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2T: Approval decision audit linkage readonly API local route mock gate`
2. `Phase ERP-Batch-2U: Approval decision audit linkage readonly API local implementation`
3. `Phase Naver-Product-Batch-2I: Product batch execution approval readonly API mock gate`
4. `Phase ERP-Multistore-2O: Invitation approval audit linkage readonly API plan`
5. `Phase ERP-Multistore-2P: Invitation approval audit linkage readonly API mock gate`

### Latest update after `Phase ERP-Batch-2T` to `Phase ERP-Multistore-2P`

Progress snapshot:

- Project overall planning progress: about `97% - 99%`
- Naver basic ERP loop progress: about `98% - 99%`
- ERP real-user landing progress: about `97% - 99%`
- Production version for long-term non-technical use: about `94% - 96%`

Implemented in this update:

- Added Codex1 local-route mock gate for approval-decision audit-linkage readonly review.
- Exposed `POST /api/v1/batch/approval-decision/audit-linkage/readonly-check` as a local readonly review route.
- Added Codex1 Naver product batch execution approval readonly API mock gate.
- Planned invitation approval audit-linkage readonly API boundaries.
- Added Codex1 invitation approval audit-linkage readonly API mock gate.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly invitation approval checklist API review, Accounts/Orders/Products readiness panels, approval-decision audit linkage readonly API review, order/product execution approval mock gates, product rollback report review, and operator-facing formal batch checklist review.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase ERP-Batch-2V: Approval decision audit linkage frontend integration plan`
2. `Phase ERP-Batch-2W: Approval decision audit linkage readonly UI integration`
3. `Phase Naver-Product-Batch-2J: Product batch execution approval readonly API local route plan`
4. `Phase ERP-Multistore-2Q: Invitation approval audit linkage readonly API local route plan`
5. `Phase ERP-Multistore-2R: Invitation approval audit linkage readonly UI plan`

### Latest update after `Phase ERP-Batch-2V` to `Phase ERP-Multistore-2R`
...
- Naver basic ERP loop progress: about `98% - 99%`
- ERP real-user landing progress: about `97% - 99%`
- Production version for long-term non-technical use: about `94% - 97%`

Implemented in this update:

- Planned Codex2 frontend integration for approval-decision audit linkage.
- Integrated Codex2 Orders with `POST /api/v1/batch/approval-decision/audit-linkage/readonly-check` and mock fallback.
- Planned the future Naver product batch execution approval readonly local route.
- Planned the future invitation approval audit-linkage readonly local route.
- Planned the future Accounts invitation approval audit-linkage readonly UI.

Current positioning:

- Suitable now: single Naver store internal trial, local product/order/inventory/sales viewing, controlled preview/single/small refresh/manual review, backup report display, restore dry-run evidence, audit trail review, role-aware action visibility, migrated auth foundation tables, readonly batch evidence API normalization, approval evidence UI display, controlled stock-only local product update, readonly membership assignment readiness checks, readonly invitation approval checklist API review, Accounts/Orders/Products readiness panels, approval-decision audit linkage readonly API review and UI display, order/product execution approval mock gates, product rollback report review, and operator-facing formal batch checklist review.
- Not suitable now: executing formal product/order batch writes, broad product writes, active multi-user login production use, sending real user invitations, real store membership assignment, large-scale multi-store production operation, automated shipment/cancel/return/exchange, real production restore, or final finance/settlement/profit use.

Next 5 recommended stages:

1. `Phase Naver-Product-Batch-2K: Product batch execution approval readonly API local route mock gate`
2. `Phase Naver-Product-Batch-2L: Product batch execution approval readonly API local implementation`
...
### Latest update after `Phase Shipping-1F` to `Phase Shipping-1J`

Progress snapshot:

- Project overall planning progress: about `98% - 99%`
- Naver basic ERP loop progress: about `98% - 99%`
- First landing Shipping Assistant feature progress: about `60% - 70%`
- ERP real-user landing progress: about `76% - 84%`
- Production version for long-term non-technical use: about `65% - 73%`

Implemented in this update:

- Added the Shipping Assistant route `/shipping`.
- Added a business-first `发货辅助` entry to the main navigation.
- Added local Naver unshipped-order candidate filtering.
- Added logistics inventory-code matching mock gate using product name plus option name.
- Added page-level manual logistics stock maintenance mock.
- Added Excel export contract preview mock for `shipping_request / xlsx`.

Current positioning:

- Suitable now: local operator walkthrough for Naver unshipped order candidates, inventory-code matching, stock readiness review, and export contract preview.
- Not suitable now: real Excel file generation, persistent logistics mapping writes, persistent logistics stock writes, export-record/audit writes, Naver shipment writeback, tracking-number upload, Coupang shipping workflow, or formal order batch sync.

Next 5 recommended stages:

1. `Phase Shipping-2A: Logistics inventory mapping schema approval plan`
2. `Phase Shipping-2B: Logistics inventory mapping schema migration`
