...
Phase 6D-6D-Fix1 keeps the real micro preview feed-only while diagnosing HTTP 400 feed parameters. Detail lookup is not called in this phase, even when `include_detail=true`. The service first tries the current feed parameter shape (`lastChangedFrom`, `lastChangedTo`, `limitCount`). If that attempt returns HTTP 400, it performs one simplified retry with only `lastChangedFrom` and `limitCount=1`, relying on Naver's documented 24-hour default window when `lastChangedTo` is omitted. The response returns sanitized diagnostics only: parameter keys, datetime format shape, presence booleans, plus-sign URL encoding safety, HTTP status, Naver error code, masked error message capped to 120 characters, and field names when available. It never returns a full URL, query values, raw error body, full order IDs, full productOrderIds, buyer/receiver names, phone numbers, addresses, delivery detail, payment raw payload, raw response bodies, tokens, authorization headers, signatures, or full channel numbers. It does not write `orders`, does not write `SyncLog`, and does not write `ApiCapabilityTestResult tested_success`.
