...
For the blocked scaffold path, the service does not create a Naver HTTP client, does not request a token, does not call `POST /v1/products/search`, does not write `products`, does not write `SyncLog`, and does not write `ApiCapabilityTestResult tested_success`. `real_preview=true` is an explicit micro-preview gate, not formal product sync availability. It is restricted to the approved local Naver store/credential, `page=1`, `size=1`, and `status` null/ALL with `REAL_API_TEST_ENABLED=true`, `REAL_API_WRITE_ENABLED=false`, decryptable credential, and configured channel number. While product search request parameters remain `request_params_confirmed=partial`, the route returns `error_code=docs_pending` before token/HTTP. If a later phase confirms the minimum request shape, the response must still return only masked sample IDs and field-observation booleans, never raw product payloads, full channel numbers, tokens, headers, signatures, long HTML, or image-detail payloads. Credential decryption only means the local encrypted secret is readable; it does not mean product API access is available.
