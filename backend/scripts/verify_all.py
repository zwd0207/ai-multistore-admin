...
EXPECTED_API_PATHS = {
    ...
    "/api/v1/sync/orders/naver/preview",
...
def verify_sync_preview_schema_and_security() -> None:
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            assert missing_order_store_id.status_code == 422, missing_order_store_id.text

            naver_order_platform_mismatch = client.post("/api/v1/sync/orders/naver/preview", json={
                "store_id": store_id,
                "start_datetime": "2026-07-01T00:00:00+09:00",
                "end_datetime": "2026-07-01T01:00:00+09:00",
            })
            assert naver_order_platform_mismatch.status_code == 400, naver_order_platform_mismatch.text
            assert naver_order_platform_mismatch.json()["error_code"] == "STORE_PLATFORM_MISMATCH", naver_order_platform_mismatch.text

            naver_order_credential_mismatch = client.post("/api/v1/sync/orders/naver/preview", json={
                "store_id": naver_store_id,
                "credential_id": other_naver_credential_id,
                "start_datetime": "2026-07-01T00:00:00+09:00",
                "end_datetime": "2026-07-01T01:00:00+09:00",
            })
            assert naver_order_credential_mismatch.status_code == 400, naver_order_credential_mismatch.text
            assert naver_order_credential_mismatch.json()["error_code"] == "credential_not_ready", naver_order_credential_mismatch.text

            naver_order_unsupported_status = client.post("/api/v1/sync/orders/naver/preview", json={
                "store_id": naver_store_id,
                "credential_id": naver_credential_id,
                "start_datetime": "2026-07-01T00:00:00+09:00",
                "end_datetime": "2026-07-01T01:00:00+09:00",
            ...
            assert naver_order_unsupported_status.status_code == 400, naver_order_unsupported_status.text
            assert naver_order_unsupported_status.json()["error_code"] == "unsupported_status_filter", naver_order_unsupported_status.text

            naver_order_long_window = client.post("/api/v1/sync/orders/naver/preview", json={
                "store_id": naver_store_id,
                "credential_id": naver_credential_id,
                "start_datetime": "2026-07-01T00:00:00+09:00",
            ...
            assert naver_order_long_window.json()["error_code"] == "date_range_invalid", naver_order_long_window.text

            naver_order_size_too_large = client.post("/api/v1/sync/orders/naver/preview", json={
                "store_id": naver_store_id,
                "credential_id": naver_credential_id,
                "start_datetime": "2026-07-01T00:00:00+09:00",
                "end_datetime": "2026-07-01T01:00:00+09:00",
            ...
            class ForbiddenNaverOrderHttpClient:
                def __init__(self, *args, **kwargs) -> None:
                    raise AssertionError("blocked Naver order preview must not create an HTTP client")

            sync_service.httpx.Client = ForbiddenNaverOrderHttpClient
            api_credential_readiness_service._request_naver_token_from_context = lambda context: (_ for _ in ()).throw(
                AssertionError("blocked Naver order preview must not request token")
            )
            try:
                with SessionLocal() as db:
                    before_order_count = len(db.scalars(select(Order).where(Order.store_id == naver_store_id)).all())
                    before_order_logs = len(db.scalars(select(SyncLog).where(SyncLog.store_id == naver_store_id)).all())
                    before_order_cap_success = len(db.execute(text(
                        "SELECT id FROM api_capability_test_results WHERE store_id = :store_id AND test_status = 'tested_success'"
                    ), {"store_id": naver_store_id}).all())
                blocked_order_preview = client.post("/api/v1/sync/orders/naver/preview", json={
                    "store_id": naver_store_id,
                    "credential_id": naver_credential_id,
                    "start_datetime": "2026-07-01T00:00:00+09:00",
                    "end_datetime": "2026-07-01T01:00:00+09:00",
                })
                assert blocked_order_preview.status_code == 200, blocked_order_preview.text
                blocked_order_data = blocked_order_preview.json()["data"]
                assert blocked_order_data["platform"] == "naver", blocked_order_data
                assert blocked_order_data["preview_type"] == "orders", blocked_order_data
                assert blocked_order_data["source_type"] == "naver_order_preview", blocked_order_data
                assert blocked_order_data["guardrail_status"] == "blocked", blocked_order_data
                assert blocked_order_data["preview_status"] == "blocked", blocked_order_data
                assert blocked_order_data["test_status"] == "not_tested", blocked_order_data
                assert blocked_order_data["error_code"] == "guardrail_blocked", blocked_order_data
                assert blocked_order_data["field_observation"]["feed_called"] is False, blocked_order_data
                assert blocked_order_data["field_observation"]["detail_called"] is False, blocked_order_data
                assert blocked_order_data["field_observation"]["orders_written"] is False, blocked_order_data
                with SessionLocal() as db:
                    assert len(db.scalars(select(Order).where(Order.store_id == naver_store_id)).all()) == before_order_count
                    assert len(db.scalars(select(SyncLog).where(SyncLog.store_id == naver_store_id)).all()) == before_order_logs
                    assert len(db.execute(text(
                        "SELECT id FROM api_capability_test_results WHERE store_id = :store_id AND test_status = 'tested_success'"
                    ), {"store_id": naver_store_id}).all()) == before_order_cap_success
            finally:
                sync_service.httpx.Client = original_http_client
                api_credential_readiness_service._request_naver_token_from_context = original_store_bound_token
            ...
            class FakeNaverOrderHttpClient:
                detail_called = False
                feed_called = False
                calls = []
                detail_payload = None
                ...
                def get(self, url: str, headers=None, params=None):
                    assert url.endswith("/v1/pay-order/seller/product-orders/last-changed-statuses"), url
                    assert "page" not in params, params
                    assert "size" not in params, params
                    assert "order_status" not in params, params
                    assert "orderStatus" not in params, params
                    assert params["limitCount"] == 1, params
                    FakeNaverOrderHttpClient.feed_called = True
                    FakeNaverOrderHttpClient.calls.append(dict(params))
                    if FakeNaverOrderHttpClient.response_sequence:
                        return FakeNaverOrderHttpClient.response_sequence.pop(0)
            ...
            try:
                ...
                feed_preview = client.post("/api/v1/sync/orders/naver/preview", json={
                    "store_id": 8,
                    "credential_id": 7,
                    "start_datetime": "2026-07-01T00:00:00+09:00",
                    "end_datetime": "2026-07-01T01:00:00+09:00",
                ...
                assert feed_preview.status_code == 200, feed_preview.text
                feed_data = feed_preview.json()["data"]
                assert feed_data["preview_status"] == "success", feed_data
                assert feed_data["test_status"] == "preview_success", feed_data
                assert feed_data["guardrail_status"] == "allowed", feed_data
                assert feed_data["field_observation"]["feed_called"] is True, feed_data
                ...
                FakeNaverOrderHttpClient.response_sequence = []
                FakeNaverOrderHttpClient.detail_called = False
                detail_preview = client.post("/api/v1/sync/orders/naver/preview", json={
                    "store_id": 8,
                    "credential_id": 7,
                    "start_datetime": "2026-07-01T00:00:00+09:00",
                    "end_datetime": "2026-07-01T01:00:00+09:00",
                ...
                for forbidden in [
                    ...
                    assert forbidden not in simplified_text, simplified_text
                ...
                FakeNaverOrderHttpClient.response_sequence = [
                    FakeNaverOrderResponse(400, {
                        ...
                        "errors": [{"field": "lastChangedFrom"}],
                    }),
                ]
                failed_preview = client.post("/api/v1/sync/orders/naver/preview", json={
                    "store_id": 8,
                    "credential_id": 7,
                    "start_datetime": "2026-07-01T00:00:00+09:00",
                    "end_datetime": "2026-07-01T01:00:00+09:00",
                ...
                assert failed_preview.status_code == 200, failed_preview.text
                failed_data = failed_preview.json()["data"]
                assert failed_data["preview_status"] == "failed", failed_data
...
def verify_operation_audit_writer_local_implementation() -> None:
    from sqlalchemy import text
    ...
    with SessionLocal() as db:
        ...
        safe_row = {
            ...
            "environment": "local",
            "actor_type": "human",
            "actor_id": "operator-safe-hash-1h",
            "actor_label": "Local operator",
            "actor_role": "owner",
            "action": "audit_writer_local_approved",
            "operation_phase": "ERP-Audit-1H",
            "correlation_id": "audit-corr-1h-chain",
            "request_id": "audit-request-1h-001",
            "status": "success",
            "reason_code": "local_writer_verified",
            "target_type": "audit_log",
            "target_id": None,
            "target_hash": "id-hash-audit1h001",
            "target_label": "Audit writer local implementation",
            "changed_field_names": ["action", "status", "safety_flags"],
            "before_summary": {
                "runtime_writer": "manual_scope_required",
                "operation_audit_logs": audit_count_before,
            },
            "after_summary": {
                "runtime_writer": "local_scope_verified",
                "operation_audit_logs_delta": 1,
            },
            "counts_summary": {
                "orders_written": 0,
            ...
            "safety_flags": {
                "local_write_scope_required": True,
                "manual_approval": True,
                "real_api_called": False,
                "raw_response_saved": False,
                "secrets_saved": False,
            ...
            "backup_path": "C:/safe-backups/codex1.db.backup-erp-audit-1h",
            "backup_sha256": valid_sha,
            "restore_source_path": "C:/safe-backups/codex1.db.backup-erp-audit-1h",
            "restore_source_sha256": restore_sha,
            "sensitive_scan_passed": True,
            "raw_response_saved": False,
            "secrets_saved": False,
            "privacy_fields_redacted": True,
            "notes": "Local writer implementation verified in temporary database only.",
        }

        not_requested_gate = write_operation_audit_log_local(
            db,
            safe_row,
            write_enabled=False,
            manual_approval=False,
        ...
        scope_gate = write_operation_audit_log_local(
            db,
            safe_row,
            write_enabled=True,
            manual_approval=True,
        ...
        assert scope_gate["skip_reason"] == "local_writer_scope_required", scope_gate
        assert scope_gate["rows_written"] == 0, scope_gate

        manual_gate = write_operation_audit_log_local(
            db,
            safe_row,
            write_enabled=True,
            manual_approval=False,
        ...
        invalid_datetime_row = dict(safe_row)
        invalid_datetime_row["created_at"] = "not-a-date"
        invalid_datetime_gate = write_operation_audit_log_local(
            db,
            invalid_datetime_row,
            write_enabled=True,
            manual_approval=True,
            local_write_scope=LOCAL_WRITER_SCOPE,
        )
        assert invalid_datetime_gate["skip_reason"] == "invalid_datetime", invalid_datetime_gate
        assert "created_at" in invalid_datetime_gate["invalid_datetime_fields"], invalid_datetime_gate

        sensitive_value_row = dict(safe_row)
        sensitive_value_row["notes"] = "010-1111-2222 must not persist"
        sensitive_value_gate = write_operation_audit_log_local(
            db,
            sensitive_value_row,
            write_enabled=True,
            manual_approval=True,
        ...
        assert sensitive_value_gate["skip_reason"] == "audit_sensitive_field_blocked", sensitive_value_gate
        sensitive_gate_text = json.dumps(sensitive_value_gate, ensure_ascii=False, default=str).lower()
        assert "010-1111-2222" not in sensitive_gate_text, sensitive_gate_text

        unsafe_key_row = dict(safe_row)
        unsafe_key_row["after_summary"] = {
            "buyer_phone": "masked value should still be blocked by unsafe key",
        }
        unsafe_key_gate = write_operation_audit_log_local(
            db,
            unsafe_key_row,
            write_enabled=True,
            manual_approval=True,
        ...
        assert unsafe_key_gate["skip_reason"] == "audit_sensitive_field_blocked", unsafe_key_gate
        assert unsafe_key_gate["rows_written"] == 0, unsafe_key_gate

        approved_gate = write_operation_audit_log_local(
            db,
            safe_row,
            write_enabled=True,
            manual_approval=True,
        ...
        assert approved_gate["status"] == "audit_row_written", approved_gate
        assert approved_gate["rows_written"] == 1, approved_gate
        assert approved_gate["runtime_writer_enabled"] is True, approved_gate
        assert approved_gate["local_write_scope"] == "approved_internal_only", approved_gate
        assert approved_gate["real_api_called"] is False, approved_gate
        assert approved_gate["sync_log_written"] is False, approved_gate

        blocked_row = dict(safe_row)
        blocked_row.update({
            "action": "audit_writer_local_blocked_evidence",
            "status": "blocked",
            "reason_code": "manual_operation_blocked",
            "before_summary": None,
            "after_summary": None,
            "counts_summary": {
                "blocked_count": 1,
        ...
        blocked_gate = write_operation_audit_log_local(
            db,
            blocked_row,
            write_enabled=True,
            manual_approval=True,
        ...
        local_rows = db.execute(text("""
            SELECT
                ...
                safety_flags
            FROM operation_audit_logs
            WHERE correlation_id = 'audit-corr-1h-chain'
            ORDER BY id
        """)).mappings().all()
        assert len(local_rows) == 2, local_rows
        assert [row["action"] for row in local_rows] == [
            "audit_writer_local_approved",
            "audit_writer_local_blocked_evidence",
        ], local_rows
        assert local_rows[0]["sensitive_scan_passed"] in (1, True), local_rows
        assert local_rows[1]["sensitive_scan_passed"] in (0, False), local_rows
        assert all(row["raw_response_saved"] in (0, False) for row in local_rows), local_rows
        assert all(row["secrets_saved"] in (0, False) for row in local_rows), local_rows
        assert all(row["privacy_fields_redacted"] in (1, True) for row in local_rows), local_rows
        ...
        for marker in FORBIDDEN_OPERATION_AUDIT_SENSITIVE_MARKERS:
            assert marker not in persisted_text, persisted_text
        assert "010-1111-2222" not in persisted_text, persisted_text
...
def verify_selected_operation_audit_runtime_wiring_mock_gate() -> None:
    from sqlalchemy import text

    from app.database import SessionLocal
    from app.services.operation_audit_service import (
        SELECTED_OPERATION_MOCK_SCOPE,
        write_selected_operation_audit_runtime_wiring_mock_gate,
    )
    ...
    base_kwargs = {
        ...
        "store_id": 8,
        "platform": "naver",
        "target_order_hash": "id-hash-audit1u-selected-order",
        "manual_approval": True,
        ...
        "verification_scope": SELECTED_OPERATION_MOCK_SCOPE,
    }
    ...
    with SessionLocal() as db:
        ...
        disabled_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
        ...
        missing_scope_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "verification_scope": None},
        )
        assert missing_scope_gate["skip_reason"] == "selected_operation_mock_scope_required", missing_scope_gate
        assert missing_scope_gate["rows_written"] == 0, missing_scope_gate

        unsupported_operation_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
        ...
        unsupported_store_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "store_id": 7},
        )
        assert unsupported_store_gate["skip_reason"] == "unsupported_store_id", unsupported_store_gate

        unsupported_platform_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "platform": "coupang"},
        )
        assert unsupported_platform_gate["skip_reason"] == "unsupported_platform", unsupported_platform_gate

        missing_hash_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "target_order_hash": ""},
        )
        assert missing_hash_gate["skip_reason"] == "missing_target_order_hash", missing_hash_gate

        missing_approval_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "manual_approval": False},
        )
        assert missing_approval_gate["skip_reason"] == "manual_approval_required", missing_approval_gate

        missing_backup_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "backup_verified": False},
        )
        assert missing_backup_gate["skip_reason"] == "pre_write_backup_required", missing_backup_gate

        missing_write_result_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "fake_write_result": None},
        )
        assert missing_write_result_gate["skip_reason"] == "missing_fake_write_result", missing_write_result_gate

        missing_readback_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{**base_kwargs, "fake_post_write_readback": None},
        )
        assert missing_readback_gate["skip_reason"] == "missing_fake_post_write_readback", missing_readback_gate

        invalid_write_status_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{
                **base_kwargs,
                "fake_write_result": {
        ...
        assert invalid_write_status_gate["skip_reason"] == "invalid_fake_write_status", invalid_write_status_gate

        invalid_verification_status_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{
                **base_kwargs,
                "fake_post_write_readback": {
        ...
        assert invalid_verification_status_gate["skip_reason"] == "invalid_fake_verification_status", invalid_verification_status_gate

        sensitive_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{
                **base_kwargs,
                "fake_post_write_readback": {
        ...
        success_gate = write_selected_operation_audit_runtime_wiring_mock_gate(db, **base_kwargs)
        assert success_gate["status"] == "audit_integration_chain_written", success_gate
        assert success_gate["phase"] == "ERP-Audit-1U", success_gate
        assert success_gate["rows_written"] == 5, success_gate
        assert success_gate["selected_operation_mock_gate"] is True, success_gate
        ...
        blocked_gate = write_selected_operation_audit_runtime_wiring_mock_gate(
            db,
            **{
                **base_kwargs,
                "target_order_hash": "id-hash-audit1u-blocked-order",
                "fake_write_result": {
                    "status": "blocked",
                    "orders_written": 0,
                    "orders_updated": 0,
        ...
        assert blocked_gate["status"] == "audit_integration_chain_written", blocked_gate
        assert blocked_gate["rows_written"] == 5, blocked_gate
        assert "local_write_blocked" in blocked_gate["chain_actions"], blocked_gate
        assert "post_write_verification_failed" in blocked_gate["chain_actions"], blocked_gate
        ...
        persisted_rows = db.execute(text("""
            SELECT
                ...
                privacy_fields_redacted
            FROM operation_audit_logs
            WHERE operation_phase = 'ERP-Audit-1U'
            ORDER BY id
        """)).mappings().all()
        ...
        assert {row["reason_code"] for row in persisted_rows} == {"selected_operation_mock_gate"}, persisted_rows
...
def verify_store_membership_assignment_mock_gate() -> None:
    ...
    from app.services import invitation_audit_linkage_service
    from app.services import permission_service
    ...
    invitation_audit_linkage_ready = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_mock_gate(
            invitation_approval=invitation_checklist_success,
            audit_linkage_context=invitation_audit_linkage_context,
            verification_scope=permission_service.VERIFICATION_SCOPE,
        )
    )
    assert invitation_audit_linkage_ready["phase"] == "ERP-Multistore-2N", invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["status"] == "user_invitation_approval_audit_linkage_mock_ready", invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["audit_linkage_ready"] is True, invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["target_store_ids"] == [8], invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["target_role"] == "operator", invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["invitation_sent"] is False, invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["users_written"] is False, invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["membership_written"] is False, invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["operation_audit_rows_planned"] is True, invitation_audit_linkage_ready
    assert invitation_audit_linkage_ready["operation_audit_rows_written"] is False, invitation_audit_linkage_ready

    invitation_audit_linkage_missing_backup = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_mock_gate(
            invitation_approval=invitation_checklist_success,
            audit_linkage_context={
                **invitation_audit_linkage_context,
                "backup_evidence_reference_planned": False,
            },
            verification_scope=permission_service.VERIFICATION_SCOPE,
        )
    )
    assert invitation_audit_linkage_missing_backup["skip_reason"] == (
        "invitation_approval_audit_linkage_incomplete"
    ), invitation_audit_linkage_missing_backup
    assert "backup_evidence_reference_planned" in invitation_audit_linkage_missing_backup["missing_linkage_flags"], (
        invitation_audit_linkage_missing_backup
    )
    assert invitation_audit_linkage_missing_backup["users_written"] is False, invitation_audit_linkage_missing_backup

    invitation_audit_linkage_sensitive = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_mock_gate(
            invitation_approval=invitation_checklist_success,
            audit_linkage_context={
                **invitation_audit_linkage_context,
                "authorization": "bearer must-not-leak-invitation-linkage",
            },
            verification_scope=permission_service.VERIFICATION_SCOPE,
        )
    )
    assert invitation_audit_linkage_sensitive["skip_reason"] == (
        "invitation_approval_audit_linkage_sensitive_material_blocked"
    ), invitation_audit_linkage_sensitive
    assert invitation_audit_linkage_sensitive["real_database_written"] is False, invitation_audit_linkage_sensitive

    invitation_audit_linkage_readonly_api_context = {
        "readonly_api_contract_planned": True,
        "business_wording_required": True,
        "technical_details_folded": True,
        "send_invitation_button_excluded": True,
    ...
    invitation_audit_linkage_readonly_api_ready = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_readonly_api_mock_gate(
            invitation_approval=invitation_checklist_success,
            audit_linkage_context=invitation_audit_linkage_context,
    ...
    assert invitation_audit_linkage_readonly_api_ready["phase"] == "ERP-Multistore-2P", (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["status"] == (
        "user_invitation_approval_audit_linkage_readonly_api_mock_ready"
    ), invitation_audit_linkage_readonly_api_ready
    assert invitation_audit_linkage_readonly_api_ready["readonly_api_mock_gate"] is True, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["backend_route_implemented"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["public_endpoint_enabled"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["invitation_sent"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["users_written"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["membership_written"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )
    assert invitation_audit_linkage_readonly_api_ready["operation_audit_rows_written"] is False, (
        invitation_audit_linkage_readonly_api_ready
    )

    invitation_audit_linkage_readonly_api_missing_fold = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_readonly_api_mock_gate(
            invitation_approval=invitation_checklist_success,
            audit_linkage_context=invitation_audit_linkage_context,
    ...
    assert invitation_audit_linkage_readonly_api_missing_fold["skip_reason"] == "readonly_api_context_incomplete", (
        invitation_audit_linkage_readonly_api_missing_fold
    )
    assert "technical_details_folded" in invitation_audit_linkage_readonly_api_missing_fold["missing_api_flags"], (
        invitation_audit_linkage_readonly_api_missing_fold
    )
    assert invitation_audit_linkage_readonly_api_missing_fold["users_written"] is False, (
        invitation_audit_linkage_readonly_api_missing_fold
    )
    ...
    invitation_audit_linkage_readonly_api_sensitive = (
        invitation_audit_linkage_service.evaluate_real_user_invitation_approval_audit_linkage_readonly_api_mock_gate(
            ...
            readonly_api_context={
                **invitation_audit_linkage_readonly_api_context,
                "authorization": "bearer must-not-leak-invitation-linkage-api",
            },
            verification_scope=permission_service.VERIFICATION_SCOPE,
        )
    )
    assert invitation_audit_linkage_readonly_api_sensitive["skip_reason"] == (
        "invitation_approval_audit_linkage_readonly_api_sensitive_material_blocked"
    ), invitation_audit_linkage_readonly_api_sensitive
    assert invitation_audit_linkage_readonly_api_sensitive["real_database_written"] is False, (
        invitation_audit_linkage_readonly_api_sensitive
    )
    ...
    serialized = json.dumps(
        {
            ...
            "invitation_audit_linkage_ready": invitation_audit_linkage_ready,
            "invitation_audit_linkage_missing_backup": invitation_audit_linkage_missing_backup,
            "invitation_audit_linkage_sensitive": invitation_audit_linkage_sensitive,
...
def verify_formal_batch_sync_production_gate() -> None:
    ...
    product_execution_readonly_api_context = {
        "readonly_api_contract_planned": True,
        "business_wording_required": True,
        "technical_details_folded": True,
        "execution_button_excluded": True,
    ...
    product_execution_readonly_api_ready = (
        sync_service.evaluate_naver_product_batch_execution_approval_readonly_api_mock_gate(
            actor_context=admin_multi_store,
            store_ids=[8],
            candidate_count=5,
            batch_size=5,
    ...
    assert product_execution_readonly_api_ready["phase"] == "Naver-Product-Batch-2I", product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["status"] == (
        "naver_product_batch_execution_approval_readonly_api_mock_ready"
    ), product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["readonly_api_mock_gate"] is True, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["backend_route_implemented"] is False, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["public_endpoint_enabled"] is False, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["execution_approved"] is False, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["products_written"] is False, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["operation_audit_rows_written"] is False, product_execution_readonly_api_ready
    assert product_execution_readonly_api_ready["formal_product_sync_open"] is False, product_execution_readonly_api_ready

    product_execution_readonly_api_missing_fold = (
        sync_service.evaluate_naver_product_batch_execution_approval_readonly_api_mock_gate(
            actor_context=admin_multi_store,
            store_ids=[8],
            candidate_count=5,
            ...
            manual_approval=True,
            execution_context=product_execution_context,
            readonly_api_context={**product_execution_readonly_api_context, "technical_details_folded": False},
            verification_scope=VERIFICATION_SCOPE,
        )
    )
    assert product_execution_readonly_api_missing_fold["skip_reason"] == "readonly_api_context_incomplete", (
        product_execution_readonly_api_missing_fold
    )
    assert "technical_details_folded" in product_execution_readonly_api_missing_fold["missing_api_flags"], (
        product_execution_readonly_api_missing_fold
    )
    assert product_execution_readonly_api_missing_fold["products_written"] is False, product_execution_readonly_api_missing_fold
    ...
    product_execution_readonly_api_sensitive = (
        sync_service.evaluate_naver_product_batch_execution_approval_readonly_api_mock_gate(
            ...
            readonly_api_context={
                **product_execution_readonly_api_context,
                "external_product_id_full": "must-not-leak-product-execution-api",
            },
            verification_scope=VERIFICATION_SCOPE,
        )
    )
    assert product_execution_readonly_api_sensitive["skip_reason"] == (
        "naver_product_batch_execution_readonly_api_sensitive_field_blocked"
    ), product_execution_readonly_api_sensitive
...
def verify_product_stock_change_and_readonly_evidence_gates() -> None:
    ...
    batch_decision_audit_linkage_api_context = {
        "readonly_api_contract_planned": True,
        "business_wording_required": True,
        "technical_details_folded": True,
        "execution_button_excluded": True,
    ...
    batch_decision_audit_linkage_api_ready = (
        sync_service.evaluate_formal_batch_approval_decision_audit_linkage_readonly_api_mock_gate(
            approval_decision=batch_decision_ready,
            audit_linkage_context=batch_decision_audit_linkage_context,
            readonly_api_context=batch_decision_audit_linkage_api_context,
            verification_scope=VERIFICATION_SCOPE,
        )
    )
    assert batch_decision_audit_linkage_api_ready["phase"] == "ERP-Batch-2R", batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["status"] == (
        "approval_decision_audit_linkage_readonly_api_mock_ready"
    ), batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["readonly_api_mock_gate"] is True, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["backend_route_implemented"] is False, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["public_endpoint_enabled"] is False, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["execution_approved"] is False, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["operation_audit_rows_written"] is False, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["orders_written"] is False, batch_decision_audit_linkage_api_ready
    assert batch_decision_audit_linkage_api_ready["products_written"] is False, batch_decision_audit_linkage_api_ready

    batch_decision_audit_linkage_api_missing_fold = (
        sync_service.evaluate_formal_batch_approval_decision_audit_linkage_readonly_api_mock_gate(
            approval_decision=batch_decision_ready,
            audit_linkage_context=batch_decision_audit_linkage_context,
            readonly_api_context={
    ...
    assert batch_decision_audit_linkage_api_missing_fold["skip_reason"] == "readonly_api_context_incomplete", (
        batch_decision_audit_linkage_api_missing_fold
    )
    assert "technical_details_folded" in batch_decision_audit_linkage_api_missing_fold["missing_api_flags"], (
        batch_decision_audit_linkage_api_missing_fold
    )
    assert batch_decision_audit_linkage_api_missing_fold["real_database_written"] is False, (
        batch_decision_audit_linkage_api_missing_fold
    )

    batch_decision_audit_linkage_api_sensitive = (
        sync_service.evaluate_formal_batch_approval_decision_audit_linkage_readonly_api_mock_gate(
            approval_decision=batch_decision_ready,
            audit_linkage_context=batch_decision_audit_linkage_context,
            readonly_api_context={
                **batch_decision_audit_linkage_api_context,
                "rawResponse": "must-not-leak-linkage-api",
            },
            verification_scope=VERIFICATION_SCOPE,
        )
    )
    assert batch_decision_audit_linkage_api_sensitive["skip_reason"] == (
        "approval_decision_audit_linkage_readonly_api_sensitive_field_blocked"
    ), batch_decision_audit_linkage_api_sensitive
    assert batch_decision_audit_linkage_api_sensitive["operation_audit_rows_written"] is False, (
        batch_decision_audit_linkage_api_sensitive
    )

    batch_decision_audit_linkage_route_mock_ready = (
        sync_service.evaluate_formal_batch_approval_decision_audit_linkage_readonly_api_local_route_mock_gate(
            approval_decision=batch_decision_ready,
            audit_linkage_context=batch_decision_audit_linkage_context,
            readonly_api_context=batch_decision_audit_linkage_api_context,
            verification_scope=VERIFICATION_SCOPE,
    ...
    assert batch_decision_audit_linkage_route_mock_ready["phase"] == "ERP-Batch-2T", batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["status"] == (
        "approval_decision_audit_linkage_readonly_api_local_route_mock_ready"
    ), batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["local_route_mock_gate"] is True, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["public_endpoint_enabled"] is False, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["backend_route_implemented"] is False, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["execution_approved"] is False, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["operation_audit_rows_written"] is False, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["orders_written"] is False, batch_decision_audit_linkage_route_mock_ready
    assert batch_decision_audit_linkage_route_mock_ready["products_written"] is False, batch_decision_audit_linkage_route_mock_ready
    ...
    with TestClient(app) as client:
        local_evidence_response = client.post("/api/v1/batch/readonly-evidence", json={
            ...
            "evidence_items": [
                ...
                {
                    "evidence_id": "naver-order-batch-readonly-alignment",
                    "store_id": 8,
                    "sync_kind": "naver_order_batch",
                    "window_label": "recent controlled order refresh candidates",
                    "candidate_count": 2,
                    "would_create": 0,
                    "would_update": 1,
                    "would_refresh_only": 1,
                    "would_skip": 0,
                    "changed_field_names": ["order_status", "delivery_status"],
                    "duplicate_check_passed": True,
                    "field_whitelist_verified": True,
                    "real_sync": False,
                    "raw_response_saved": False,
                    "privacy_fields_redacted": True,
                    "formal_sync_open": False,
                    "business_message": "Order batch readonly evidence is normalized for manual review only.",
                    "next_action": "order_batch_refresh_approval_required",
                }
            ],
        })
        ...
        local_audit_linkage_response = client.post("/api/v1/batch/approval-decision/audit-linkage/readonly-check", json={
            "approval_decision": local_decision,
            "audit_linkage_context": batch_decision_audit_linkage_context,
            "readonly_api_context": batch_decision_audit_linkage_api_context,
        })
        assert local_audit_linkage_response.status_code == 200, local_audit_linkage_response.text
        local_audit_linkage = local_audit_linkage_response.json()["data"]
        assert local_audit_linkage["phase"] == "ERP-Batch-2U", local_audit_linkage
        assert local_audit_linkage["status"] == "approval_decision_audit_linkage_readonly_api_ready", local_audit_linkage
        assert local_audit_linkage["approval_decision_audit_linkage_readonly_api_local"] is True, local_audit_linkage
        assert local_audit_linkage["backend_route_implemented"] is True, local_audit_linkage
        assert local_audit_linkage["public_endpoint_enabled"] is True, local_audit_linkage
        assert local_audit_linkage["route_path"] == "/api/v1/batch/approval-decision/audit-linkage/readonly-check", local_audit_linkage
        assert local_audit_linkage["execution_approved"] is False, local_audit_linkage
        assert local_audit_linkage["operation_audit_rows_written"] is False, local_audit_linkage
        assert local_audit_linkage["real_database_written"] is False, local_audit_linkage
        assert local_audit_linkage["orders_written"] is False, local_audit_linkage
        assert local_audit_linkage["products_written"] is False, local_audit_linkage
        assert local_audit_linkage["sync_log_written"] is False, local_audit_linkage
        assert local_audit_linkage["capability_tested_success_written"] is False, local_audit_linkage
        ...
        batch_execution_write_boundary_context = {
        ...
        batch_execution_write_boundary_ready = (
            sync_service.evaluate_formal_batch_execution_write_boundary_approval_plan(
                execution_approval=batch_execution_approval_route,
                write_boundary_context=batch_execution_write_boundary_context,
                verification_scope=VERIFICATION_SCOPE,
            )
        )
        assert batch_execution_write_boundary_ready["phase"] == "ERP-Batch-4A", (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["status"] == (
            "formal_batch_execution_write_boundary_approval_plan_ready"
        ), batch_execution_write_boundary_ready
        assert batch_execution_write_boundary_ready["write_boundary_plan_ready"] is True, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["private_helper_only"] is True, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["backend_route_implemented"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["public_endpoint_enabled"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["store_ids"] == [8], batch_execution_write_boundary_ready
        assert batch_execution_write_boundary_ready["sync_kinds"] == [
            "naver_order_batch",
            "naver_product_batch",
        ], batch_execution_write_boundary_ready
        assert batch_execution_write_boundary_ready["targets"] == ["orders", "products"], (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["required_actions"] == [
            "orders.batch_sync_write",
            "products.batch_sync_write",
        ], batch_execution_write_boundary_ready
        assert batch_execution_write_boundary_ready["total_candidate_count"] == 7, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["total_would_update"] == 1, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["changed_fields"] == ["order_status"], (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["execution_approved"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["batch_execution_enabled"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["write_endpoint_enabled"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["real_api_called"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["real_database_written"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["orders_written"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["products_written"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["operation_audit_rows_written"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["formal_sync_open"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["platform_writes_enabled"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["raw_response_saved"] is False, (
            batch_execution_write_boundary_ready
        )
        assert batch_execution_write_boundary_ready["privacy_fields_redacted"] is True, (
            batch_execution_write_boundary_ready
        )

        batch_execution_write_boundary_missing = (
            sync_service.evaluate_formal_batch_execution_write_boundary_approval_plan(
                execution_approval=batch_execution_approval_route,
        ...
        assert batch_execution_write_boundary_missing["phase"] == "ERP-Batch-4A", (
            batch_execution_write_boundary_missing
        )
        assert batch_execution_write_boundary_missing["skip_reason"] == "write_boundary_context_incomplete", (
            batch_execution_write_boundary_missing
        )
        assert "rollback_required" in batch_execution_write_boundary_missing["missing_write_boundary_flags"], (
            batch_execution_write_boundary_missing
        )
        assert batch_execution_write_boundary_missing["real_database_written"] is False, (
            batch_execution_write_boundary_missing
        )

        batch_execution_write_boundary_sensitive = (
            sync_service.evaluate_formal_batch_execution_write_boundary_approval_plan(
                execution_approval=batch_execution_approval_route,
                write_boundary_context={
                    **batch_execution_write_boundary_context,
                    "rawResponse": "must-not-leak-write-boundary",
                },
                verification_scope=VERIFICATION_SCOPE,
            )
        )
        assert batch_execution_write_boundary_sensitive["phase"] == "ERP-Batch-4A", (
            batch_execution_write_boundary_sensitive
        )
        assert batch_execution_write_boundary_sensitive["skip_reason"] == (
            "formal_batch_execution_write_boundary_sensitive_field_blocked"
        ), batch_execution_write_boundary_sensitive
        assert batch_execution_write_boundary_sensitive["real_database_written"] is False, (
            batch_execution_write_boundary_sensitive
        )

        batch_execution_write_boundary_write_attempt = (
            sync_service.evaluate_formal_batch_execution_write_boundary_approval_plan(
                execution_approval=batch_execution_approval_route,
                write_boundary_context={
                    **batch_execution_write_boundary_context,
        ...
        assert batch_execution_write_boundary_write_attempt["phase"] == "ERP-Batch-4A", (
            batch_execution_write_boundary_write_attempt
        )
        assert batch_execution_write_boundary_write_attempt["skip_reason"] == (
            "side_effect_not_allowed_in_write_boundary_plan"
        ), batch_execution_write_boundary_write_attempt
        assert batch_execution_write_boundary_write_attempt["blocked_flag"] == "write_endpoint_enabled", (
            batch_execution_write_boundary_write_attempt
        )
        ...
        assert batch_execution_write_boundary_oversized["phase"] == "ERP-Batch-4A", (
            batch_execution_write_boundary_oversized
        )
        assert batch_execution_write_boundary_oversized["skip_reason"] == (
            "write_boundary_candidate_count_exceeds_limit"
        ), batch_execution_write_boundary_oversized
        assert batch_execution_write_boundary_oversized["real_database_written"] is False, (
            batch_execution_write_boundary_oversized
        )
        ...
        local_audit_linkage_sensitive_response = client.post(
            "/api/v1/batch/approval-decision/audit-linkage/readonly-check",
            json={
                "approval_decision": local_decision,
                "audit_linkage_context": {
                    **batch_decision_audit_linkage_context,
                    "rawResponse": "must-not-leak-local-linkage",
                },
                "readonly_api_context": batch_decision_audit_linkage_api_context,
            },
        )
        assert local_audit_linkage_sensitive_response.status_code == 200, local_audit_linkage_sensitive_response.text
        local_audit_linkage_sensitive = local_audit_linkage_sensitive_response.json()["data"]
        assert local_audit_linkage_sensitive["phase"] == "ERP-Batch-2U", local_audit_linkage_sensitive
        assert local_audit_linkage_sensitive["skip_reason"] == "approval_decision_audit_linkage_sensitive_field_blocked", (
            local_audit_linkage_sensitive
        )
        assert local_audit_linkage_sensitive["operation_audit_rows_written"] is False, local_audit_linkage_sensitive
        assert local_audit_linkage_sensitive["real_database_written"] is False, local_audit_linkage_sensitive
    ...
    serialized = json.dumps(
        {
            ...
            "batch_decision_audit_linkage_api_ready": batch_decision_audit_linkage_api_ready,
            "batch_decision_audit_linkage_api_missing_fold": batch_decision_audit_linkage_api_missing_fold,
            "batch_decision_audit_linkage_api_sensitive": batch_decision_audit_linkage_api_sensitive,
            "batch_decision_audit_linkage_route_mock_ready": batch_decision_audit_linkage_route_mock_ready,
            ...
            "batch_execution_write_boundary_ready": batch_execution_write_boundary_ready,
            "batch_execution_write_boundary_missing": batch_execution_write_boundary_missing,
    ...
    with SessionLocal() as db:
        ...
        missing_rollback_flag = sync_service._evaluate_naver_product_batch_rollback_drill_mock_gate(
            product_write_summary=write_result,
            backup_evidence=backup_evidence,
            rollback_checklist={**rollback_checklist, "rollback_sql_reviewed": False},
            verification_scope=VERIFICATION_SCOPE,
        )
        assert missing_rollback_flag["skip_reason"] == "rollback_checklist_incomplete", missing_rollback_flag
        assert missing_rollback_flag["rollback_executed"] is False, missing_rollback_flag
        assert "rollback_sql_reviewed" in missing_rollback_flag["missing_checklist_flags"], missing_rollback_flag

        restore_blocked = sync_service._evaluate_naver_product_batch_rollback_drill_mock_gate(
            product_write_summary=write_result,
            backup_evidence=backup_evidence,
            rollback_checklist={**rollback_checklist, "real_restore_requested": True},
            verification_scope=VERIFICATION_SCOPE,
        )
        assert restore_blocked["skip_reason"] == "real_restore_request_not_allowed_in_mock_gate", restore_blocked
        assert restore_blocked["real_restore_executed"] is False, restore_blocked
        assert restore_blocked["production_db_touched"] is False, restore_blocked

        sensitive_rollback = sync_service._evaluate_naver_product_batch_rollback_drill_mock_gate(
            product_write_summary={**write_result, "rawResponse": "must-not-leak"},
            backup_evidence=backup_evidence,
            rollback_checklist=rollback_checklist,
            verification_scope=VERIFICATION_SCOPE,
        )
        assert sensitive_rollback["skip_reason"] == "rollback_drill_sensitive_field_blocked", sensitive_rollback
        ...
        assert rollback_ready["rollback_drill_ready"] is True, rollback_ready
        assert rollback_ready["backup_evidence_verified"] is True, rollback_ready
        assert rollback_ready["updated_count"] == 3, rollback_ready
        assert rollback_ready["created_count"] == 0, rollback_ready
        assert rollback_ready["stock_only_write_verified"] is True, rollback_ready
        assert rollback_ready["rollback_executed"] is False, rollback_ready
    ...
    for forbidden in [
        ...
        assert forbidden not in rollback_serialized, rollback_serialized
...
def verify_shipping_mapping_schema_and_local_write() -> None:
    ...
    with TestClient(app) as client:
        ...
        safe_tracking_row = {
            "order_reference": "shipping-order-safe-001",
            "product_order_reference": "product-order-safe-001",
            "logistics_inventory_code": "PXG-WHEEL-BAG-BK-OS",
            "carrier": "Korea carrier",
            "tracking_number": "TRK202607050001",
            "shipped_at": "2026-07-05T18:00:00+09:00",
            "operator_note": "mock parser only",
        }
        tracking_manual_gate = evaluate_tracking_number_import_mock_gate(
            store_id=store_id,
            platform="naver",
        ...
        assert tracking_manual_gate["phase"] == "Shipping-4B", tracking_manual_gate
        assert tracking_manual_gate["status"] == "blocked", tracking_manual_gate
        assert tracking_manual_gate["skip_reason"] == "manual_approval_required", tracking_manual_gate
        assert tracking_manual_gate["file_parsed"] is False, tracking_manual_gate
        assert tracking_manual_gate["tracking_numbers_written"] is False, tracking_manual_gate
        assert tracking_manual_gate["shipment_writeback_called"] is False, tracking_manual_gate
        assert shipping_counts(store_id) == excel_gate_before_counts, shipping_counts(store_id)

        tracking_sensitive_gate = evaluate_tracking_number_import_mock_gate(
            store_id=store_id,
            platform="naver",
            tracking_rows=[{
                **safe_tracking_row,
        ...
        assert tracking_sensitive_gate["status"] == "blocked", tracking_sensitive_gate
        assert tracking_sensitive_gate["skip_reason"] == "tracking_import_sensitive_field_blocked", tracking_sensitive_gate
        assert tracking_sensitive_gate["tracking_numbers_written"] is False, tracking_sensitive_gate
        assert tracking_sensitive_gate["real_database_written"] is False, tracking_sensitive_gate
        assert shipping_counts(store_id) == excel_gate_before_counts, shipping_counts(store_id)

        tracking_ready_gate = evaluate_tracking_number_import_mock_gate(
            store_id=store_id,
            platform="naver",
        ...
        assert tracking_ready_gate["status"] == "tracking_import_mock_parse_ready", tracking_ready_gate
        assert tracking_ready_gate["file_type"] == "tracking_upload", tracking_ready_gate
        assert tracking_ready_gate["file_format"] == "xlsx", tracking_ready_gate
        assert tracking_ready_gate["row_count"] == 2, tracking_ready_gate
        assert tracking_ready_gate["ready_row_count"] == 1, tracking_ready_gate
        assert tracking_ready_gate["duplicate_row_count"] == 1, tracking_ready_gate
        assert tracking_ready_gate["tracking_number_import_open"] is False, tracking_ready_gate
        assert tracking_ready_gate["tracking_numbers_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["shipment_writeback_called"] is False, tracking_ready_gate
        assert tracking_ready_gate["import_record_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["real_database_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["real_api_called"] is False, tracking_ready_gate
        assert tracking_ready_gate["orders_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["products_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["sync_log_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["capability_tested_success_written"] is False, tracking_ready_gate
        assert tracking_ready_gate["raw_response_saved"] is False, tracking_ready_gate
        assert tracking_ready_gate["secrets_saved"] is False, tracking_ready_gate
        assert tracking_ready_gate["privacy_fields_redacted"] is True, tracking_ready_gate
        ...
        history_before_counts = shipping_counts(store_id)
        ...
        assert history_payload["phase"] == "Shipping-4D", history_payload
        assert history_payload["status"] == "shipping_export_history_ready", history_payload
        assert history_payload["export_history_readonly"] is True, history_payload
        assert history_payload["total"] == 1, history_payload
        assert history_payload["items"][0]["file_sha256"] == export_result["file_sha256"], history_payload
        assert history_payload["items"][0]["row_count"] == 1, history_payload
        assert history_payload["items"][0]["rows"][0]["logistics_inventory_code"] == "PXG-WHEEL-BAG-BK-OS", history_payload
        assert history_payload["real_database_written"] is False, history_payload
        assert history_payload["real_api_called"] is False, history_payload
        assert history_payload["orders_written"] is False, history_payload
        assert history_payload["products_written"] is False, history_payload
        assert history_payload["sync_log_written"] is False, history_payload
        assert history_payload["capability_tested_success_written"] is False, history_payload
        assert history_payload["tracking_number_import_open"] is False, history_payload
        assert history_payload["shipment_writeback_open"] is False, history_payload
        assert shipping_counts(store_id) == history_before_counts, shipping_counts(store_id)

        tracking_route_response = client.post("/api/v1/shipping/tracking-import/mock-parse", json={
            "store_id": store_id,
            "platform": "naver",
        ...
        assert tracking_route_response.status_code == 200, tracking_route_response.text
        tracking_route_payload = tracking_route_response.json()["data"]
        assert tracking_route_payload["status"] == "tracking_import_mock_parse_ready", tracking_route_payload
        assert tracking_route_payload["row_count"] == 1, tracking_route_payload
        assert tracking_route_payload["tracking_number_import_open"] is False, tracking_route_payload
