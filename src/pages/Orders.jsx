...
const api = { list: dataProvider.getOrders, create: mockApi.createOrder, update: mockApi.updateOrder, remove: mockApi.deleteOrder };
const columns = [
  { key: 'orderNo', title: '订单编号', render: (value) => <strong>{value}</strong> }, { key: 'product', title: '商品' }, { key: 'store', title: '所属店铺' },
  { key: 'customer', title: '客户' }, { key: 'phone', title: '联系电话' }, { key: 'amount', title: '订单金额', render: (value, row) => `${Number(value).toLocaleString()} ${row.currency || 'KRW'}` },
...
export default function Orders() { return <ResourcePage title="订单管理" description="跟踪各店铺订单履约、配送与退款状态" resourceName="订单" api={api} columns={columns} fields={fields} statuses={['待发货', '배송중', '구매확정', '取消退款']} initialForm={{ orderNo: '', product: '', store: '', customer: '', amount: 0, status: '', createdAt: '' }} readOnly={isBackendSource} />; }
