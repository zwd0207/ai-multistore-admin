...
export function AuthPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      else { await login({ loginIdentifier: identifier, password }); }
    } catch (nextError) { setError(nextError.message || '暂时无法完成验证，请稍后重试。'); }
...
function AuthShell({ children }) { return <main className="auth-shell"><section className="auth-panel">{children}</section></main>; }
