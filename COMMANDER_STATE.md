...
- Production cutover completed on `2026-07-17` from the approved `19:00 Asia/Shanghai` window. The backend cutover release remains `820bfab6c4cc23fd716f2fb61e2070d1780720e1` with T24 capability-metadata hotfix `d175991fc59a202c772dcbd4e0d99e41df439126`; the production frontend includes authenticated-acceptance hotfix `f3036e32321756997e5b54f6bd720289b97d6838`. GitHub Actions runs `29572971660`, `29582534495`, `29584540587`, and `29585205365`, local `verify_all.py`, server build, Sol security review, disposable migration, and isolated production startup all passed.
...
- Keep real platform writes, real customer replies, and real shipment dispatch disabled. The owner-selected PXG Naver probe and the separately approved one-time 30-day inquiry import both passed. Do not repeat the import, enable persistent inquiry reads, or start automatic synchronization without a new explicit approval.
...
1. **P0 logistics correctness:** add exact compatibility for verified legacy Naver order identifiers, prove batch rollback on every association failure, and re-enable only the two blocked logistics checkpoints after focused T17-T24 and PostgreSQL tests pass.
2. **P0 dual-store stability:** observe both stores through repeated order, inquiry, product, and repaired logistics cycles; require cursor completion, zero cross-store effects, safe retry behavior, and a fresh encrypted backup.
3. **P1 operator readiness:** repeat authenticated desktop and 390px QA against the real two-store data state, including store switching, inquiry classification/conversation display, automatic-read attention, and logistics freshness messaging.
4. **P1 invitations and email:** enable the already implemented invitation, MFA, password-reset, and alert-mail flow only after the dual-store readonly gate is stable. Keep public self-registration closed for the first operator rollout.
5. **P2 manual writes:** accept customer reply first, then shipment writeback as a separate gate. Each requires its own real single-record approval, idempotency/reconciliation proof, backup health, and all readonly resources fresh. Never enable both for an initial trial.
6. **P3 Windows Ziniao helper:** keep T19/T20 local-only until the core Naver operator workflow is accepted. Do not make Ziniao desktop opening a production-server dependency.
