...
- Production cutover completed on `2026-07-17` from the approved `19:00 Asia/Shanghai` window. The running release is `820bfab6c4cc23fd716f2fb61e2070d1780720e1`; GitHub Actions run `29572971660`, local `verify_all.py`, server build, Sol security review, disposable migration, and isolated production startup all passed.
...
- The one bootstrap platform-administrator invitation is pending. Its token is stored only in protected server evidence and a restricted local operator file. The owner must accept it, set a password, enroll TOTP MFA, and retain recovery codes before authenticated desktop/mobile and tenant-isolation acceptance can continue.
- OSS `DeleteObject` permission for the backup prefix remains pending. Backup creation and restore are operational, but retention cleanup cannot be accepted until the role can delete the two private preflight objects and a delete probe passes.
