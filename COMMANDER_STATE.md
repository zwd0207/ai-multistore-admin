...
- The owner replaced the earlier 22:00 window and approved the production cutover for `2026-07-17 19:00 Asia/Shanghai`. The bootstrap administrator identity and private OSS bucket were supplied out of repository; their exact values must remain in protected runtime configuration and must not be committed to this public repository.
- Commits `ce0c579` and `917961a` close the T23/T24 authentication, tenant-selection, readonly-store gate, lease-renewal, retry, environment-enum, and database-uniqueness release blockers. The required Alembic head is now `d4b7a91c2e6f`.
- Safe next action is push and CI verification of `release/operator-v1`, followed by a read-only server release rehearsal and OSS/RAM-role preflight. Do not stop the service, copy rows, bootstrap the administrator, switch `DATABASE_URL`, or enable the backup timer before the approved window.
- At the approved window, follow `ops/T22_POSTGRES_MIGRATION_RUNBOOK.md` exactly. Preserve the untouched SQLite source and the prior release for rollback, and keep all platform writes disabled throughout migration and acceptance.
- Server staging for `cc6d91d` passed the production build, bundle gate, Alembic `d4b7a91c2e6f` rehearsal, actual-data disposable migration, and isolated production-runtime health check. The candidate environment is protected and default-closed. OSS list access works, but `GetBucketAcl` remains denied until the RAM role policy is corrected; do not declare T22 complete without an encrypted upload and restore drill.
